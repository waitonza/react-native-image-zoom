...
var ImageViewer = (function (_super) {
    ...
    ImageViewer.prototype.componentWillMount = function () {
        ...
        this.imagePanResponder = react_native_1.PanResponder.create({
            ...
            onStartShouldSetPanResponderCapture: function (evt, gestureState) { return setResponder; },
            onMoveShouldSetPanResponder: function (evt, gestureState) { return setResponder; },
            onMoveShouldSetPanResponderCapture: function (evt, gestureState) { return setResponder; },
            ...
            onPanResponderGrant: function (evt, gestureState) {
                ...
                if (evt.nativeEvent.changedTouches.length > 1) {
                    _this.centerDiffX = (evt.nativeEvent.changedTouches[0].pageX + evt.nativeEvent.changedTouches[1].pageX) / 2 - _this.props.cropWidth / 2;
                    _this.centerDiffY = (evt.nativeEvent.changedTouches[0].pageY + evt.nativeEvent.changedTouches[1].pageY) / 2 - _this.props.cropHeight / 2;
                }
                // 计算长按
                ...
                if (evt.nativeEvent.changedTouches.length <= 1) {
                    // 一个手指的情况
                    if (new Date().getTime() - _this.lastClickTime < 175) {
                        // 认为触发了双击
                        _this.lastClickTime = 0;
            ...
            onPanResponderMove: function (evt, gestureState) {
                if (evt.nativeEvent.changedTouches.length <= 1) {
                    // x 位移
                    var diffX = gestureState.dx - _this.lastPositionX;
                    if (_this.lastPositionX === null) {
                        diffX = 0;
                    }
                    // y 位移
                    var diffY = gestureState.dy - _this.lastPositionY;
            ...
            onPanResponderRelease: function (evt, gestureState) {
                ...
                if (evt.nativeEvent.changedTouches.length === 1 && stayTime < _this.props.leaveStayTime && moveDistance < _this.props.leaveDistance) {
                    _this.props.onClick();
                }
                else {
                    _this.props.responderRelease(gestureState.vx);
                }
                if (_this.scale < 1) {
                    // 如果缩放小于1，强制重置为 1
                    _this.scale = 1;
                ...
                _this.props.onScale(_this.scale);
                // 水平溢出量置空
                _this.horizontalWholeOuterCounter = 0;
                // 取消长按
                if (_this.longPressTimeout) {
    ...
    ImageViewer.prototype.render = function () {
        ...
        return (React.createElement(react_native_1.View, __assign({ style: [image_zoom_style_1["default"].container, { width: this.props.cropWidth, height: this.props.cropHeight }] }, this.imagePanResponder.panHandlers),
